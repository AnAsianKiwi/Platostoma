...
export const Library: React.FC<LibraryProps> = ({ 
  ...
  const performSelectionLogic = (data: { left: number, top: number, width: number, height: number, containerRect: DOMRect }) => {
    ...
    itemRefs.current.forEach((el, id) => {
        ...
        const intersects = !(box.left > itemAbs.right || box.right < itemAbs.left || box.top > itemAbs.bottom || box.bottom < itemAbs.top);
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    e.preventDefault();
    e.stopPropagation();
    startPoint.current = { x: e.clientX - rect.left + container.scrollLeft, y: e.clientY - rect.top + container.scrollTop };
    ...
    if (!isDragging.current) {
        if (clickedItemRef.current) {
            ...
            if (e.ctrlKey || e.metaKey) {
             ...
             if (!e.ctrlKey && !e.shiftKey) { setSelectedIds(new Set()); setIsSelectionMode(false); }
        }
    }
  ...
  return (
    <div 
        ...
        className="w-full h-full flex flex-col px-6 pb-6 overflow-y-auto relative select-none [scrollbar-gutter:stable] focus:outline-none"
        onMouseDown={handleMouseDown}
    >
      <style>{`
        @keyframes pop-highlight {
          0% { transform: scale(1); border-color: #4ade80; box-shadow: 0 0 0 0 rgba(74, 222, 128, 0.7); }
          50% { transform: scale(1.02); border-color: #4ade80; box-shadow: 0 0 20px 10px rgba(74, 222, 128, 0); }
          100% { transform: scale(1); border-color: inherit; box-shadow: none; }
        }
        .animate-pop {
          animation: pop-highlight 0.8s ease-out;
          z-index: 20 !important; /* Lowered to 20 */
        }
      `}</style>

      {/* SELECTION BOX: Lowered to z-30 */}
      <div 
        ref={selectionBoxRef}
        className="absolute z-30 bg-blue-500/20 border border-blue-400 pointer-events-none"
        style={{ display: 'none', top: 0, left: 0, width: 0, height: 0, willChange: 'transform, width, height' }}
      />

      {/* STICKY HEADER: Lowered to z-40 (was 60) */}
      <div className="sticky top-0 z-40 pt-6 pb-4 -mx-6 px-6 bg-slate-950 border-b border-slate-800/50 mb-4 shadow-lg" data-no-drag>
      ...
      {filteredItems.length === 0 ? (
        <div className="text-center py-20 text-slate-500 bg-slate-900/50 rounded-xl border border-slate-800 border-dashed mt-10">
              ...
              return (
                <div 
                  ...
                  className={`
                    ...
                    ${isHighlighted ? 'animate-pop' : ''} 
                    ${isSelected 
                      ? 'border-osmanthus-500 ring-2 ring-osmanthus-500/50 shadow-[0_0_20px_rgba(78,153,86,0.5)] transform scale-[0.98] z-10' 
