...
export const mockSearchMedia = async (query: string): Promise<MockSearchResult[]> => {
  ...
  if (q.includes('one piece') || q.includes('naruto') || q.includes('bleach') || q.includes('fate')) {
    return [
      { title: query, type: 'Anime', coverUrl: `https://picsum.photos/300/450?random=${Math.random()}`, year: 2000 },
      { title: query, type: 'Manga', coverUrl: `https://picsum.photos/300/450?random=${Math.random()}`, year: 1999 },
      { title: `${query} (Movie)`, type: 'Movie', coverUrl: `https://picsum.photos/300/450?random=${Math.random()}`, year: 2010 },
    ];
  }
  ...
  return [{
    title: query,
    type: randomType,
    coverUrl: `https://picsum.photos/300/450?random=${Math.random()}`,
    year: 2023
  }];
};
